...
* Make the element with both the `avatar` and `proportioned` classes 300 pixels wide, then give it a height so that it retains its (don't hardcode in a original square proportions pixel value for the height!).
